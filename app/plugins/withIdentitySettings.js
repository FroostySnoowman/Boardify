...
module.exports = function withIdentitySettings(config, props = {}) {
  ...
  const buildNumber = props.buildNumber || config.ios?.buildNumber || config.runtimeVersion?.version || '1';
  ...
  config = withDangerousMod(config, [
    ...
    (config) => {
      ...
      const schemePath = path.join(config.modRequest.projectRoot, 'ios', `${projectName}.xcodeproj`, 'xcshareddata', 'xcschemes', `${projectName}.xcscheme`);

      try {
        if (fs.existsSync(schemePath)) {
          const xml = fs.readFileSync(schemePath, 'utf8');
          const updated = xml.replace(/(<LaunchAction[^>]*buildConfiguration\s*=\s*")Debug(")/, '$1Release$2');
          if (updated !== xml) {
