...
import { listMembers, Member, listLineupEntries, LineupEntry, LineupFormat, addLineupEntry } from '../src/api/teams';
...
export default function AddLineupPlayerScreen() {
  const insets = useSafeAreaInsets();
  const params = useLocalSearchParams();
  const teamId = params.teamId as string;
  const lineupId = params.lineupId as string;
  const lineupName = params.lineupName as string | undefined;
  const format = (params.format as LineupFormat) || 'singles';
  ...
  const showSuccessBanner = useCallback((player1Name: string, player2Name: string) => {
    ...
    // Delay animation start to ensure the Animated.View has mounted after state update
    setTimeout(() => {
      Animated.parallel([
    ...
    const loadData = async () => {
      ...
      try {
        const [membersResult, entriesResult] = await Promise.all([
          listMembers(teamId),
          listLineupEntries(teamId, lineupId, format)
        ]);
  ...
  const handleAddPlayer = async (memberId: string, partnerId?: string) => {
    setAdding(memberId);
    try {
      const entry = await addLineupEntry(teamId, lineupId, memberId, format, partnerId);
        ...
        <View style={{ maxWidth: 768, alignSelf: 'center', width: '100%' }}>
          {lineupName && (
            <View className="mb-4 p-4 rounded-xl bg-white/5 border border-white/10">
              ...
              <Text className="text-white font-semibold text-lg">{decodeURIComponent(lineupName)}</Text>
            </View>
          )}
            ...
            >
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={{ color: '#86efac', fontSize: 13, marginTop: 2 }}>
                  {successTeam.player1} and {successTeam.player2} have been added together to this lineup.
                </Text>
              </View>
            </Animated.View>
          )}
            ...
            {loading ? (
              ...
              <View className="gap-3">
                <Text className="text-gray-400 text-sm mb-2">
                  {isDoubles
                    ...
                    : 'Select a player to add to the lineup'}
                </Text>
              ...
              <View className="py-12 items-center">
                ...
                <Text className="text-gray-500 text-sm mt-2 text-center px-4">
                  All eligible members are already in this lineup, or there are no team members with a playable role (Player, Coach, or Owner).
                </Text>
                <TouchableOpacity
                  onPress={() => router.back()}
                  activeOpacity={0.8}
