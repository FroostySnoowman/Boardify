...
import { signInWithEmail, signUpWithEmail, signInWithGoogle, signInWithApple, forgotPassword, verifyResetCode, resetPassword } from '../src/api/auth';
...
export default function LoginScreen() {
  const { user, loading: authLoading, setUserContext } = useAuth();
  ...
  // After OAuth redirect, session cookie is set; AuthContext loads user — redirect to home
  useEffect(() => {
    if (Platform.OS === 'web' && !authLoading && user) {
      router.replace('/(tabs)');
    }
  }, [authLoading, user]);
