...
export function MobileTopNav({
  ...
  const getPageName = React.useMemo(() => {
    if (pathname === '/settings') {
      return 'Settings';
    } else if (pathname === '/profile') {
      return 'Profile';
    } else if (pathname === '/log-match') {
      return 'Log Match';
    } else if (pathname === '/new-event') {
      return 'New Event';
    } else if (pathname === '/create-team') {
      return 'Create Team';
    }
    
    const path = segments.join('/');
    
    if (path.includes('spectate')) {
      return 'Spectate';
    ...
    } else if (path === '' || path.includes('index') || path.includes('(tabs)')) {
      return 'Home';
    }
      ...
      >
        ...
        <View className="flex-1 flex-row items-center justify-between px-4">
          <View className="flex-row items-center">
            ...
            >
              <Text className="text-xl font-extrabold text-white tracking-tight">{getPageName}</Text>
            </Pressable>
          </View>
