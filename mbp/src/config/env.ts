...
type EnvConfig = {
  ...
  INCLUDE_SANDBOX_SUBSCRIPTIONS: boolean;
...
export const ENV: EnvConfig = {
  ...
  APNS_ENVIRONMENT: (() => {
    const v = getEnvVar('EXPO_PUBLIC_APNS_ENVIRONMENT', '');
    if (v === 'sandbox' || v === 'production') return v;
    return __DEV__ ? 'sandbox' : 'production';
  })(),
  INCLUDE_SANDBOX_SUBSCRIPTIONS: getEnvVar('EXPO_PUBLIC_INCLUDE_SANDBOX_SUBSCRIPTIONS', 'false') === 'true',
