  ...
  const formatted = parts.map(part => {
    ...
    if (!match) {
      ...
      if (ampmMatch) {
        ...
        return preventAmPmWrap(d.toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' }));
      }
      return trimmed;
    }
...
/** Replace space before AM/PM with non-breaking space so the suffix doesn't wrap to two lines. */
function preventAmPmWrap(s: string): string {
  return s.replace(/\s+([AP]M)$/i, '\u00A0$1');
}
