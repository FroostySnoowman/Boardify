...
export const getImageUrl = (src: string | null | undefined): string | null => {
  ...
  if (src.startsWith('http://') || src.startsWith('https://')) {
    // Fix localhost for Android - use 10.0.2.2 for emulator
  ...
  // Fix localhost for Android - use 10.0.2.2 for emulator, or detect actual IP
  if (Platform.OS === 'android' && API_BASE.includes('localhost')) {
    // Replace localhost with 10.0.2.2 (Android emulator's special IP for host machine)
